...
public:
    ...
    cv::Mat deserialize_image(redisReply* reply) {
        ...
        chrono_time = std::chrono::system_clock::time_point(std::chrono::nanoseconds(serializableMat.timestamp()));

        cv::Mat img;

        try {
            img = cv::Mat(serializableMat.rows(), serializableMat.cols(), serializableMat.elt_type());
